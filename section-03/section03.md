...
- 브라우저에서는 JSX 문법을 지원하지 않기 때문에 변환 과정(컴파일)이 필요할 것이다.
...
- 이때 React는 브라우저가 읽을 수 있는 코드로, JSX 문법으로 작성된 코드를 브라우저가 지원하는 코드로 변환(컴파일)을 해주어야 한다.

### JSX 문법은 어떻게 브라우저가 읽을 수 있는 코드로 변형되는 걸까?
...
- 이처럼 우리가 JSX 문법으로 코드를 작성하면 JSX 문법을 지원하지 않는 브라우저에서 읽히지 못하므로, JSX 코드로 작성을 하면 React 프로젝트 setup(바벨)에서 자동으로 변형(컴파일)되어 브라우저에게 읽히게 된다.
...
- 프로젝트의 규모가 커질 수록 그리고 그에 따른 컴포넌트가 많이 추가될 수록 하위 폴더로 정리하게 될 것이다.
- 하위 폴더로 정리하는 이유는 컴포넌트를 체계화시키고, 모든 컴포넌트들을 하나의 폴더 안에서 관리하지 않기 위해서다.
