...
- 지금까지 리액트가 어떻게 상태(state)를 관리하는지에 대해 알아보았다. 하지만 앞서 거론했듯이 강조해야 할 중요한 개념이 있다. 그것은 리액트가 상태(state)에 대한 '갱신'을 어떻게 하는지에 대한 것 말이다.
...
- 예시 코드를 한 번 보자. 우리는 `MyProduct` 컴포넌트가 존재한다. 이 컴포넌트 안에서는 `useState` 훅을 통해 일부 상태(state)들을 관리할 수 있다. 즉, 리액트가 상태(state)를 관리해줄 수 있다. `MyProduct` 컴포넌트 내부의 `currentState` 상태(state)는 'DVD' 라는 문자열 초기값을 가졌다고 가정해보자. 아마도 어떤 어플리케이션이 될지는 모르겠지만 사용자가 버튼을 누르거나 하면서 해당 컴포넌트 안에서 상태(state)가 업데이트되고 갱신될 것이다. 그리고 상태(state) 업데이트 함수인 `setCurrentState` 를 통해서 새로운 값으로 업데이트 될 것이다.
...
- 그리고 이제 이 제품을 'DVD'에서 'BOOK'으로 변경하고자 한다. `buttonClickHandler` 함수에서 발생하는 일은 `setCurrentState` 가 실행되었음에도 기존의 `cuurentState` 초기 값인 'DVD' 가 즉각적으로 변경되지 않는다는 것이다. 대신에 상태(state) 업데이트 함수인 `setCurrentState` 를 호출하면, 'BOOK' 데이터로 상태(state) 업데이트를 하도록 예약을 하게 된다. 이것이 상태(state) 갱신 예약이다.

### 상태(state) 갱신 예약이란 무엇인가

- 리액트는 상태(state) 업데이트 함수인 `setCurrentState` 를 호출하고 상태(state) 업데이트가 필요하다는 것을 인지하고 있고 또한 처리할 계획까지 준비하고 있지만 이를 즉시 처리하지는 않는다. 사실 대부분의 경우에는 상태(state) 변경이 발생하면 상태(state) 갱신에 대한 스케줄 작업을 매우 빠르게 발생한다. 거의 즉각적인 거나 다름 없다. 실제로는 순간적으로 발생하는 것처럼 보이며, 사용자 역시 이러한 '지연'에 대해서는 알지 못한다. 하지만 리액트는 이 상태(state) 변화를 '지연' 시키게 된다.

### 리액트는 상태(state) 변화를 지연 시킨다

- 대다수의 성능 기반의 작업들은 거의 동시에 진행되는데, 잠재적으로 리액트는 더 높은 우선 순위를 갖는 것으로 간주한다. 화면에 사용자가 입력할 수 있는 인풋 창이 있다고 가정해보자. 사용자 입력에 응답하는 것이 화면의 문자를 변경하는 것보다는 더 우선 순위가 높을 것이다. 이러한 이유로 리액트는 예정된 상태(state) 변경을 '지연' 시킬 수 있다.

### 리액트는 상태(state) 변화가 발생하면 우선순위에 따라 같은 타입임을 보증한다

- 리액트가 하는 일은 상태(state) 변화가 발생하게 되면 이 상태(state) 변화의 순서를 명확하게 해서, 같은 타입 임을 보증하는 것이다. 우리가 앞서 살펴보았던 코드를 다시 보자.
...
- 즉각 실행이 필수적인 것은 아니다. 결국 언젠가는 처리가 되고 상태(state)는 'BOOK'으로 바뀔 것이다. 그리고 상태(state)가 활성화되면 또 상태(state) 변화가 처리되면 리액트는 컴포넌트를 재평가하고 컴포넌트 함수를 재실행할 것이다.

### state의 스케쥴링

- 상태(state)의 스케줄링 때문에 다수의 상태(state) 갱신 예약이 동시에 있을 가능성도 있다. 이 때문에 동시에 여러 번의 갱신이 스케줄 될 수 있으므로, 상태(state)를 갱신할 때는 함수 형태를 이용해서 갱신하는 것을 추천한다. 특히 이전 상태(state) 스냅샷에 의존해야 한다면 말이다.
...
- 해당 코드는 다른 예시지만, 함수 형태로 상태(state)를 갱신하고 있기에 가져왔다. 사실 많은 경우에 함수 형태로 상태(state)를 갱신하지 않는다고 큰 문제가 되는 일은 별로 없다. 또한 굉장히 빠르게 처리가 되기 때문에 리액트에서 스케줄링으로 인한 '지연' 조차 사용자는 느끼지 못할 가능성이 더 크다. 하지만 이론상으로는 분명 이 스케줄링 작업으로 인한 '지연'이 발생될 수 있으므로 함수 형태로 업데이트 하는 것이 상태(state) 변경이 순서대로 처리되고 이전 상태(state)를 기반으로 매 회 상태(state) 변경이 발생할 때마다 가상 최신의 상태(state)를 얻을 수 있게 해주는 안전한 방법인 것은 명백할 것이다.

### 함수 형태로 상태(state)를 업데이트하지 않는다면 어떤 결과를 초래할까?

- 만약 함수 형태로 상태(state)를 업데이트하지 않는다면 발생할 수 있는 어떤 가능성이란 무엇일까? 컴포넌트 함수가 마지막으로 실행될 때의 상태(state)를 얻는다는 것은 즉 상태(state) 변경이 순서대로 실행될 때와는 조금 다른 결과를 낳을 가능성이 높다는 뜻이다. 해당 컴포넌트의 재 렌더링 주기는 모두 동일하고 완료되지 않은 상태(state) 변경 작업이 여러 개 있다고 생각해보자. 해당 컴포넌트 내부에 있는 모든 코드들은 최종 컴포넌트 스냅샷으로부터 발생하지만, 당연히 상태(state) 변경 작업이 처리된 경우에는 컴포넌트는 중간에 다시 렌더링 될 것이다. 하지만 상태(state) 변경 작업들은 이미 갱신에 대한 예약이 되어 있기 때문에 완료되지 않은 상태(state) 변경 작업은 새로운 컴포넌트의 결과를 고려하지 않게 된다.

### 상태(state) 업데이트는 함수 형태로 !

- 상태(state) 업데이트는 함수 형태로 사용해야 리액트가 미완료된 상태(state) 변경 작업에 대하여 최신의 상태(state)를 사용하고 컴포넌트가 재렌더링되었을 그 시점의 상태(state)를 사용하지 않을 수 있게 된다. 이것이 함수 형태를 사용하길 권장하는 이유가 된다. 컴포넌트가 재렌더링 되었을 때의 시점과 상태(state) 변경이 예약되는 시점의 차이를 이해하는 것은 중요하다. 하나의 컴포넌트가 재평가 되면서 미완료된 상태(state) 변경 작업이 여러 개 있을 수 있다. 그리고 이것이 바로 함수 형태로 상태(state) 업데이트를 해야 하는 핵심적인 이유가 된다.

### 동일한 이슈(가장 최신의 상태(state)를 받아오지 않을 가능성)를 처리하는 또 다른 방법

- 우리가 이전에 `useEffect`에 대해 학습했던 적을 기억할 것이다.
  > [section 10 참조](https://github.com/ichbinmin2/udemy-react-ts-redux-nextjs-study/blob/main/section-10/section10.md#useReducer-&-useEffect)
...
- 이전의 코드에서 `useEffect` 내부에서 이메일과 비밀번호의 유효 상태(state)를 통해 폼의 유효성을 업데이트하는 것이 좀 더 안전하다고 했던 것을 기억할 것이다. 이전 상태(state)의 스냅샷을 기반으로 상태(state)를 갱신하기 위해 함수 형식을 사용하는 것과 동일하게 `useEffect` 상태(state) 또는 종속된 값이 변경될 때마다 의존성 메커니즘을 통해 내부에서 선언한 이펙트가 재실행 되도록 하는 방법 말이다. 이를 통해서 컴포넌트 함수가 재실행되면서 매번 리펙트 역시 다시 재실행되기 때문에 미완료된 상태(state) 변경 작업이 빠짐없이 실행된다. 그리고 실제로 이펙트는 이 이후에 실행되기에 이런 작업이 수행될 때마다 가장 최신의 결과를 얻을 수 있게 된다.

### 두 가지 방법 중에 어떤 것을 선택해서 사용해야 할까?

- 이 두가지 방법은 모두 '가장 최신의 상태(state)를 받아올 수 있도록' 하는 방법들이다. 그리고 이 두가지 방법 중에 어떤 걸 선택하느냐는 우리가 이를 통해서 무엇을 얻고 싶은지에 따라서 달라진다.

### 상태(state) 갱신 스케줄링을 기억해야만 하는 이유

- 상태(state) 갱신 스케줄링은 꼭 기억해야만 하는 메커니즘이다. 단순히 이 메커니즘을 통해 무언가를 하려고 하는 게 아니며 사실상 스케줄 업데이트 관리는 리액트가 해주고 있지만 말이다. 다만 우리가 데이터 없이 작업을 할 때 발생할 수 있는 잠재적인 위험을 배제하기 위해서 반드시 짚고 넘어가야하는 개념이다. 그리고 이 잠재적 이슈를 방지하기 위해 우리가 선택할 수 있는 두개의 가능성(함수 형태로 상태(state) 업데이트하기, `useEffect`를 사용하기) 또한 기억해야 한다.
...
- 예시 코드를 보자. `exampleHandler` 함수 내부의 두 개의 상태(state) 갱신이 스케줄링 될 것이다. 하지만 컴포넌트 역시 재실행되고 재평가된다는 것도 의미한다. 사실 이것들은 리액트가 하는 역할이 아니다. 이렇게 두 개의 상태(state) 갱신이 같은 코드 조각에 존재한다면, 즉 같은 함수가 서로 다른 프로미스에 있지 않고, 같은 곳에 존재한다면 이 둘 사이에 시간 '지연'과 같은 현상은 발생하지 않는다.
...
- 두 개의 상태(state) 갱신 함수가 같은 곳(`exampleHandler`)에 위치하고 이 둘 모두 지금과 같이 상태(state) 갱신 함수를 호출한다면 리액트는 이들에 대한 상태(state) 갱신을 하나의 동기화 프로세스에서 같이 실행하게 된다. 예를 들어서, 하나의 함수가 처음부터 끝까지 어떠한 콜백이나 프로미스 없이 실행된다면 리액트는 이 함수로부터 발생하는 모든 상태(state) 갱신 작업을 하나의 상태(state) 갱신 작업으로 처리한다는 뜻이다.
